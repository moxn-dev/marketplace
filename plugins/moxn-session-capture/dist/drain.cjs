...
function writeSpool(dir, rec) {
  ...
  (0, import_node_fs2.writeFileSync)(tmp, JSON.stringify(merged));
  (0, import_node_fs2.renameSync)(tmp, fileFor(dir, rec.sessionId));
}
function readSpool(dir) {
  if (!(0, import_node_fs2.existsSync)(dir)) return [];
  return (0, import_node_fs2.readdirSync)(dir).filter((f) => f.endsWith(".json")).flatMap((f) => {
    try {
      return [JSON.parse((0, import_node_fs2.readFileSync)((0, import_node_path2.join)(dir, f), "utf-8"))];
    } catch {
      return [];
    }
  });
...
function staleSpooledSessions(dir) {
  return readSpool(dir).filter((r) => {
    if (r.lastSyncedAt == null) return true;
    try {
      return Math.floor((0, import_node_fs2.statSync)(r.transcriptPath).mtimeMs) / 1e3 > r.lastSyncedAt;
    } catch {
      return false;
    }
  });
...
// ../../src/lib/sessions/drain.ts
async function drainSpool(opts) {
  const sync = opts.sync ?? syncTranscript;
  let ok = 0;
  for (const rec of staleSpooledSessions(opts.spoolDir)) {
    try {
      await sync(rec.transcriptPath, {
        baseUrl: opts.baseUrl,
        apiKey: opts.apiKey,
        status: "complete"
      });
      markSynced(opts.spoolDir, rec.sessionId);
      ok++;
    } catch (e) {
      console.error("[drain]", rec.sessionId, e.message);
    }
  }
  return ok;
}
...
function spoolFromPayload(stdin, deps) {
  if (deps.debugDir) {
    try {
      (0, import_node_fs3.mkdirSync)(deps.debugDir, { recursive: true });
      (0, import_node_fs3.writeFileSync)((0, import_node_path3.join)(deps.debugDir, `payload-${Date.now()}.json`), stdin);
    } catch {
    }
  }
  let p;
...
async function main() {
  ...
  if (self) {
    try {
      ...
      console.error("[session-capture:drain] self-sync", e.message);
    }
  }
...
main().catch((e) => console.error("[session-capture:drain]", e.message));
